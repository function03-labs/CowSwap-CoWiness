...
def compute_cowiness_detailed(tx_hash):
    ...
    try:
        ...
        # Calculate the total USD volume of tokens traded in and out of the transaction
        total_volume_in_usd = sum(
            [vol["usd_value"] for _, vol in volume_in_usd.items()]
        )
        total_volume_out_usd = sum(
            [vol["usd_value"] for _, vol in volume_out_usd.items()]
        )
